...
Google, Kakao 등 소셜 로그인 버튼은 각 사의 브랜드 가이드라인이 색상을 고정해서, 앱 색상 토큰 대신 브랜드 고정값을 그대로 사용한다.

| 버튼   | 배경        | 텍스트          | 참고                                              |
| ------ | ----------- | ---------------- | -------------------------------------------------- |
| Google | `#FFFFFF`   | `#1F1F1F`         | 로고 형태/색상 변경 금지, 테두리는 선택(다크 배경 위라 생략) |
| Kakao  | `#FEE500`   | `#000000`         | 로고 형태/색상 변경 금지, 컨테이너 radius 12px 고정 |

두 버튼 모두 컨테이너 radius는 12px(`rounded-xl`)로 통일한다 (Google은 브랜드 규정 없음, Kakao 규정에 맞춤).
