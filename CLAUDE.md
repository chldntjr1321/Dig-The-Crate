...
### Phase 2 (진행 중) — LP 플레이어 관련 작업

- LP 플레이어 UI + 애니메이션 최적화 (#44, 완료)
- 미니 플레이어 재생 큐 / 이전·다음 곡 이동 (#55, 완료)
- iTunes 미리듣기 매칭 정확도 개선 + 앨범 전곡 재생 (#57)
- Web Worker + Canvas 앨범 커버 색상 추출 (#45, 완료)

### 보류 중 (Phase 미배정)

- 이미지 최적화 (LazyImage 등) — 아직 착수 전. 성능 문제를 체감한 후 진행 (`docs/WORKFLOW.md` 참조)

**진행 중인 Phase 범위 밖 기능을 미리 구현하지 않는다.**
